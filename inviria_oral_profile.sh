...
for FILE in "${FILES[@]}"; do
    if [ -n "$FILE" ]; then
        sylph profile -c 50 --min-number-kmers 10 --min-count-correct 1 --min-spacing 10 HOVD-geneseqences_200.syldb  "$FILE" -t "$THREADS" > temporary/"$(basename "$FILE" | sed 's/.*//')_sylph.tsv"
    else
        sylph profile -c 50 --min-number-kmers 10 --min-count-correct 1 --min-spacing 10 HOVD-geneseqences_200.syldb  -1 "$R1" -2 "$R2" -t "$THREADS" > temporary/"$(basename "$R1" | sed 's/R1.*//')_sylph.tsv"
    fi
done

sylph-tax taxprof temporary/*tsv -t hovd_annotations_taxonomy_filt.tsv
mv *.sylphmpa temporary/
sylph-tax merge temporary/*.sylphmpa --column relative_abundance -o merged_abundance_hovd_host.tsv
