...
for FILE in "${FILES[@]}"; do
    if [ -n "$FILE" ]; then
        sylph profile -c 50 --min-number-kmers 10 --min-count-correct 1 --min-spacing 10 votus_full_100.syldb "$FILE" -t "$THREADS" > temporary/"$(basename "$FILE" | sed 's/.*//')_sylph.tsv"
    else
        sylph profile -c 50 --min-number-kmers 10 --min-count-correct 1 --min-spacing 10 votus_full_100.syldb -1 "$R1" -2 "$R2" -t "$THREADS" > temporary/"$(basename "$R1" | sed 's/R1.*//')_sylph.tsv"
    fi
done
