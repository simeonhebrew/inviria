...
for FILE in "${FILES[@]}"; do
    if [ -n "$FILE" ]; then
        sylph profile -c 50 --min-number-kmers 10 --min-count-correct 1 --min-spacing 10 votus_full_100.syldb "$FILE" -t "$THREADS" > ../results/temporary/"$(basename "$FILE" | sed 's/.*//')_sylph.tsv"
    else
        sylph profile -c 50 --min-number-kmers 10 --min-count-correct 1 --min-spacing 10 votus_full_100.syldb -1 "$R1" -2 "$R2" -t "$THREADS" > ../results/temporary/"$(basename "$R1" | sed 's/R1.*//')_sylph.tsv"
    fi
done

sylph-tax taxprof ../results/temporary/*tsv -t ../taxonomy/uhgv_taxonomy_file.tsv
mv *.sylphmpa ../results/temporary/
sylph-tax merge ../results/temporary/*.sylphmpa --column relative_abundance -o ../results/merged_abundance_uhgv_host.tsv
